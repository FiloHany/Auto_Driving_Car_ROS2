...
class NoisyController(Node):
    ...
    def jointCallback(self,msg):
        #Adding some Gaussian noise to positions dp_left, dp_right  => in order to add noise to the reading.
        wheel_encoder_left = msg.position[1] + np.random.normal(0, 0.005) # 0 => mean , 0.005 => std
        wheel_encoder_right = msg.position[0] + np.random.normal(0, 0.005) 
        # Contain the variation  of the position of left wheel postion[1] => postion of left wheel at the current moment in time ...noise was added 
